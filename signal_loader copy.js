...
// FETCH SHEET
// -----------------------------
async function fetchSheet(spreadsheetId, range, token) {
...
async function loadAllSignals() {
  ...
  for (let i = 1; i < surveyRows.length; i++) {
    ...
    const id = clean(surveyRows[i][0]);        // channel_id
    const val = parseFloat(surveyRows[i][2]);  // survey_signal
    const ts = new Date(surveyRows[i][3]).getTime(); // timestamp
